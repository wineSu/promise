...
function resolve(promise, value){
	...
	} else if (value instanceof Promises) {
...
function handleThenable(promise, value, then){
	let settled = false; //2.3.3.3.3 如果resolvePromise和rejectPromise都被调用，或其中一个被调用了多次，则以第一次调用的为准，并忽略之后的调用。
	try {
...
Promises.resolve = function(value){
	return new Promises(function(resolve){resolve(value)})
}
Promises.reject = function(reason){
	return new Promises(function(resolve, reject){reject(reason)})
}
Promises.all = function(arr){
	let newPormise = new Promises(noop),
		value = [],
		num = 0;
	if (({}).toString.call(arr) === '[object Array]') {
		try{
			arr.forEach(function(k, index){
				if (k instanceof Promises) {
